...
export function SignupStep2Screen({ navigation, route }: Props) {
  const { email, nickname, password } = route.params;
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <View style={styles.progressWrap}>
          {[1, 2, 3].map(i => <View key={i} style={[styles.bar, i <= 2 && styles.barActive]} />)}
        </View>
        <Text style={styles.stepLabel}>2/3</Text>
      </View>

      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.eyebrow}>STEP 02 · 주기</Text>
        ...
        <View style={styles.calCard}>
          <View style={styles.calHeader}>
            ...
            <View style={styles.navRow}>
              ...
              <TouchableOpacity style={[styles.navBtn, isCurrentMonth && styles.navBtnDisabled]} onPress={nextMonth} disabled={isCurrentMonth}>
                <Text style={styles.navIcon}>›</Text>
              </TouchableOpacity>
            </View>
          </View>
            ...
            {grid.map((d, i) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  disabled={isFuture}
                >
                  <Text style={[styles.dayText, isSelected && styles.dayTextSelected, isFuture && styles.dayTextFuture]}>
                    {d}
                  </Text>
        ...
        <View style={styles.selectedCard}>
          ...
          <Text style={styles.daysAgo}>
            {year === todayY && month === todayM ? `${todayD - selectedDay}일 전` : ''}
          </Text>
        </View>
      </ScrollView>

      <View style={styles.footer}>
        <PrimaryButton onPress={() => navigation.navigate('SignupStep3', { email, nickname, password, lastPeriodDate: selectedDate })}>
          다음
        </PrimaryButton>
      </View>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  calTitle: { fontSize: 13, fontWeight: '800', color: Colors.ink1 },
  ...
  weekDay: { width: CELL_SIZE, textAlign: 'center', fontSize: 10, fontWeight: '700', color: Colors.ink3, letterSpacing: 0.6 },
  ...
  dayText: { fontSize: 13, fontWeight: '500', color: Colors.ink1 },
  dayTextSelected: { color: Colors.coral, fontWeight: '900' },
  ...
  selectedCard: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: Colors.bgInk, borderRadius: 16, padding: 16, marginTop: 14 },
  selectedLabel: { fontSize: 10, fontWeight: '700', color: 'rgba(242,238,232,0.5)', letterSpacing: 1.2 },
  selectedDate: { fontSize: 18, fontWeight: '900', color: Colors.inkInv, marginTop: 2 },
